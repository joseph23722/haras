  ...
  <div class="card mb-4">
    ...
    <div class="card-body text-center" style="background-color: #f9f9f9;">
      ...
      <button class="btn btn-outline-danger btn-lg me-3" style="border-color: #dc3545;" data-bs-toggle="modal" data-bs-target="#modalSalidaAlimento">
        <i class="fas fa-arrow-down"></i> Registrar Movimientos Alimentos
      </button>
...
<!-- SOLO SE NECESITARA UN MODAL PARA ENTRADA Y SALIDAS DE ALIMENTOS -->
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header" style="background: #ff6b6b; color: white;">
          <h5 class="modal-title" id="modalSalidaAlimentoLabel">Registrar Movimientos Alimento</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
              ...
              <div class="col-md-6">
                <div class="form-floating">
                  <select id="tipoMovimiento" name="tipoMovimiento" class="form-select" required>
                    <option value="">Seleccione un Tipo de Movimiento</option> <!-- Cargar dinamicamente Entrada y Salida desde la base de datos -->
                  </select>
                  <label for="alimento-select-salida">Tipo de Movimiento</label>
                </div>
              </div>
              <div class="col-md-6">
                <div class="form-floating">
                  <select id="nombreProducto" name="nombreProducto" class="form-select" required>
                    <option value="">Seleccione un Producto</option> <!-- Cargar dinamicamente Productos dependiendo el tipomovimiento -->
                    <!-- Si es salida los productos que me muestran solo tiene que ser los que cuentan con stock -->
                    <!-- Si es entrada que cargue absolutamente todos los productos registrados -->
                    <!-- Al listar el producto tanto para entrada como salida, al costado tiene que mostrarme la cantidad total que hay para un mejor manejo -->
                    <!-- La lista al momento de dar salida tiene que estar mostrando el nombre del Producto mas el lote -->
                  </select>
                  <label for="alimento-select-salida">Seleccione un Producto</label>
                </div>
              </div>
              ...
              <div class="col-md-6">
                <div class="form-floating">
                  <select id="tipoEquinoMovimiento" name="idTipoEquino" class="form-select" required>
                    ...
                    <!-- En tipo Equino o Cantidad Equinos tienen que cargar los 6 estados (Servidas, Vacias, Preñadas, Padrillos activos, inactivos) que te mencioné más
                          La cantidad que hay, Por ejemplo: Vacias: 10, Preñadas 29, que ambos datos guarde tal y como mostraría -->
                    <!-- Además solo se cargará cuando su status muestre que este vivo -->
                    <!-- Este campo será necesario solo para SALIDAS, para entradas no debe de aparecer -->
                  </select>
                  <label for="tipoEquinoMovimiento">Seleccione Tipo de Equino</label>
                </div>
              </div>

              <div class="col-md-6">
                <div class="form-floating">
                  <input type="number" name="cantidad" id="cantidad" placeholder="" class="form-control" required min="1">
                  <label for="cantidad-salida">Cantidad</label>
                  <!-- Cantidad con la que se realizara el movimiento tanto entrada como salida -->
                  <!-- Solo aceptará números positivos ademas de que  -->
                </div>
              </div>
              ...
              <div class="col-md-6">
                <div class="form-floating">
                  <select id="unidadMedidaSalida" name="unidadMedida" class="form-select" required>
                    <option value="">Seleccione la Unidad de Medida</option>
                    <!-- Nose si sea necesario para salidas o para entradas, pero si llega a ser necesario lo colocas -->
                  </select>
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
          <button type="button" class="btn btn-danger" id="guardarSalida">Guardar Salida</button> <!-- Has un solo modal para el movimiento de alimentos tanto entrada como salida -->
        </div>
      </div>
    </div>
  </div>
    ...
    document.addEventListener("DOMContentLoaded", () => {
      ...
      alimentoSelectEntrada.addEventListener("change", async function() {
        ...
        if (nombreAlimento) {
          ...
          unidadMedidaSelectEntrada.innerHTML = '<option value=""></option>';
        }
      });
      ...
      alimentoSelectSalida.addEventListener("change", async function() {
        ...
        if (nombreAlimento) {
          ...
          // Si no se ha seleccionado un alimento, limpiar el select de unidades de medida
          unidadMedidaSelect.innerHTML = '<option value="">Seleccione un a</option>';
        }
      });
      ...
      const loadAlimentos = async () => {
        try {
          ...
          `).join('');
            ...
            alimentoSelectEntrada.innerHTML = '<option value="">Seleccione un Tipo de Movimiento</option>';
            alimentoSelectSalida.innerHTML = '<option value="">Seleccione un tipo Movimiento</option>';
      ...
      const loadHistorialMovimientos = async () => {
        try {
          ...
          const responseEntradas = await fetch(entradasURL, {
            method: "GET"
          });
          ...
          const responseSalidas = await fetch(salidasURL, {
            method: "GET"
          });
